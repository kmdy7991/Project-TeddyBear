...
const App: React.FC = () => {
  ...
  const onRecAudio = () => {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then((stream) => {
      ...
      analyser.onaudioprocess = function (e: any) {
        if (e.playbackTime > 180) { // 녹음 시간이 180초를 초과하면
  ...
  const uploadAudioFile = async () => {
    if (audioUrl) {
      ...
      formData.append("text", "I can also read and write Korean"); // 추가 데이터 FormData에 추가

      try {
        const response = await fetch("http://localhost:8000/upload/", { // 서버에 POST 요청
          method: "POST",
          body: formData,
        });
  
        if (!response.ok) { // 요청 실패 시
  ...
  return (
    <>
      ...
      <button onClick={play} disabled={!audioUrl}>재생</button>
      <button onClick={uploadAudioFile} disabled={!audioUrl}>발음 평가</button>
    </>
  );
};
